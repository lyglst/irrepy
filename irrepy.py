...
def gen_iso_input(filename, supercell, kpt):
    ...
    newcell = spglib.standardize_cell(cell, symprec=1e-4)
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "hi:k:s:o:")
    except getopt.GetoptError:
        print("""
            irrepy -i vasprun.xml -k kpoints list -s supercell vector -o output directory
            e.g. irepy -i vasprun.xml -k 'GM','X','R' -s 1,0,0,0,1,0,0,0,1 -o irrep_modes
            """)
    for opt, arg in opts:
        if opt=='-h':
            print('irrepy -i vasprun.xml -k kpoints list -s supercell vector -o output directory')
        elif opt=='-i':
            filename = arg
        elif opt=='-k':
            kpt = arg.split(',')
        elif opt=='-s':
            supercell = np.reshape(list(map(int, arg.split(','))), (3,3))
        elif opt=='-o':
            boutput = True
            if arg!='':
                foutput = arg
    ...
    for file in input_list:
        ...
        lname += list(map(lambda x: x+'_'+file.split('_')[2]+'_'+file.split('_')[3], tname))
        lpt += tpt
        lpv += tpv
    ...
    if boutput:
        for i, name in enumerate(lnmode, 0):
            with open(foutput+'/'+name, 'w+') as f:
                for coor in lvec[i]:
                    f.write('{0:2.2f}       {1:2.2f}        {2:2.2f} \n'.format(coor[0], coor[1], coor[2]))

    for i in range(len(normal_modes)):
        print('mode {0} ({1:2.4f}cm-1) is composed of:'.format(i, freq[i]))
        for j,vec in enumerate(lvec):
            if np.abs(np.dot(np.reshape(normal_modes[i], -1), np.reshape(vec, -1)))>0.05:
